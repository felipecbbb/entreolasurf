...
async function fetchActivity(slug) {
  ...
  const [packs, photos, testimonials, faqs] = await Promise.all([
    supabase.from('activity_packs').select('*').eq('activity_id', activity.id).order('sessions'),
    ...
    return `
      <article class="pack-card${p.featured ? ' featured' : ''}">
        ...
        <div class="pack-card-cta">
          <button class="btn red" data-reserve-class="${a.id}" data-class-name="${esc(a.nombre)} × ${n}" data-class-price="${price}" data-class-sessions="${n}" data-class-type="${typeKey}">Reservar ${deposit}€</button>
