...
// Inicia el pago ONLINE (Stripe) del saldo pendiente de un bono. Es opcional:
// el cliente puede pagarlo aquí o en la escuela. El webhook actualiza total_paid
// y marca las inscripciones como pagadas tras el cobro.
export async function startBonoBalanceCheckout(bonoId, amount, label = '') {
  ...
  const resp = await supabase.functions.invoke('create-checkout', {
    body: {
      items: [{
        id: `bono-balance-${bonoId}`,
        type: 'bono_balance',
        name: `Saldo pendiente${label ? ' · ' + label : ''}`,
        price: Number(amount),
        quantity: 1,
        metadata: { bonoId },
      }],
