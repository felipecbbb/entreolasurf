...
function renderPrice(c) {
  ...
  if (priceBig) {
    ...
    if (c.old_price && Number(c.old_price) > Number(c.price)) {
      const old = Number(c.old_price).toLocaleString('es-ES');
      const savings = Number(c.old_price) - Number(c.price);
...
function renderSoldOut(c) {
  const isPast = c.date_start && c.date_start < new Date().toISOString().slice(0, 10);
