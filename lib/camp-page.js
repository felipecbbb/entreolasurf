...
import { initCampCarousel } from '/lib/camp-carousel.js';
...
function renderPhotos(c) {
  // Carrusel: si el camp tiene fotos en el panel, mandan sobre las del HTML.
  // Si no tiene, se quedan las del HTML (respaldo), no se vacía la galería.
  const track = document.querySelector('.camp-carousel-track');
  if (track) {
    if (c.photos.length) {
      track.innerHTML = c.photos.map(p =>
        `<figure class="camp-slide"><img src="${esc(p.url)}" alt="${esc(p.alt_text || c.title)}" loading="lazy" decoding="async"></figure>`
      ).join('');
    }
    initCampCarousel(track.closest('.camp-carousel'));
    return;
  }

  // Páginas antiguas con la tira de fotos
