...
/* ---------- Header: drawer + iconos (sustituye el dropdown del carrito) ---------- */
export function initCartDrawer() {
  document.querySelectorAll('.cart-nav').forEach(nav => {
    nav.classList.remove('has-dd');
    nav.querySelector('.dropdown')?.remove();
    const link = nav.querySelector('a');
    if (!link) return;
    // Reemplaza "Cart ▾" por un icono limpio + contador
    const pill = link.querySelector('.cart-pill');
    const count = pill ? pill.textContent : '0';
    link.innerHTML = `
      ...
      <span class="cart-pill">${count}</span>`;
    link.setAttribute('aria-label', 'Abrir carrito');
    link.addEventListener('click', (e) => { e.preventDefault(); openCartDrawer(); });
  });
  updateCartPill();

  // Pequeño icono en "Mi cuenta" para un header más cuidado
  document.querySelectorAll('.main-nav .nav-item > a[href="/mi-cuenta/"]').forEach(a => {
    if (a.querySelector('.nav-account-icon')) return;
    a.classList.add('nav-account-link');
    a.insertAdjacentHTML('afterbegin', `<svg class="nav-account-icon" width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9"><circle cx="12" cy="8" r="4"/><path d="M4 21v-1a6 6 0 016-6h4a6 6 0 016 6v1"/></svg> `);
  });
}
