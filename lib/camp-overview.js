...
async function init() {
  const { data: camps } = await supabase
    .from('surf_camps')
    .select('slug, sold_out, spots_taken, max_spots, price, status, hero_image, date_start')
    .order('date_start');
  ...
  const isDatePast = (d) => d && new Date(d).getTime() < todayMs;
  const isDateFuture = (d) => d && new Date(d).getTime() >= todayMs;
  ...
  for (const c of camps) {
    ...
    if (!isSoldOut && isDateFuture(c.date_start) && c.status !== 'coming_soon') {
      nextCampSlug = c.slug;
      break;
    }
  }
  ...
  document.querySelectorAll('.camp-card').forEach(card => {
    ...
    const isPast = isDatePast(camp.date_start);
