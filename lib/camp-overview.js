...
async function init() {
  const { data: camps } = await supabase
    .from('surf_camps')
    .select('slug, sold_out, spots_taken, max_spots, price, old_price, status, hero_image')
    .order('date_start');
  ...
  document.querySelectorAll('.camp-card').forEach(card => {
    ...
    if (priceEl && camp.price) {
      ...
      if (camp.old_price && Number(camp.old_price) > Number(camp.price)) {
        const old = Number(camp.old_price).toLocaleString('es-ES');
        priceEl.innerHTML = `<span class="old-price" style="color:rgba(255,255,255,.7)">${old}€</span> ${price}€`;
      } else {
