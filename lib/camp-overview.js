...
async function init() {
  const { data: camps, error } = await supabase
    .from('surf_camps')
    .select('slug, sold_out, spots_taken, max_spots, price, original_price, deposit, status, hero_image, date_start, date_end')
    .order('date_start');
  ...
  document.querySelectorAll('.camp-card').forEach(card => {
    ...
    // Hide expired camps (date_end already passed)
