...
async function init() {
  ...
  const WA_PHONE = '34634466130';
  const waHref = (text) => `https://wa.me/${WA_PHONE}?text=${encodeURIComponent(text)}`;
  let soldOutCount = 0;   // para ajustar el box de lista de espera
  ...
  document.querySelectorAll('.camp-card').forEach(card => {
    ...
    if (isSoldOut) {
      ...
      // Replace buttons. El SOLD OUT solo cerraba la puerta: al lado va un aviso
      // para quedarse con quien llega tarde (si no, se va sin dejar rastro).
      const actions = card.querySelector('.camp-actions');
      if (actions) {
        const fecha = dateRange || camp.title || 'el Surf Camp';
        actions.innerHTML = '<span class="btn disabled" style="width:100%;text-align:center">SOLD OUT</span>'
          + `<a class="btn camp-card-notify" href="${waHref(`¡Hola! El Surf Camp del ${fecha} está agotado. Avisadme de la siguiente fecha.`)}" target="_blank" rel="noopener">Avísame de la siguiente fecha</a>`;
      }
    } else if (hasOffer) {
      // --- OFERTA badge ---
      if (existingBadge) {
  ...
  // --- Box de lista de espera: el copy cambia si ya hay camps agotados ---
  const box = document.getElementById('camp-waitlist');
  if (box && soldOutCount > 0) {
    box.classList.add('is-urgent');
    const set = (id, text) => { const el = document.getElementById(id); if (el) el.textContent = text; };
    set('camp-waitlist-kicker', soldOutCount > 1 ? 'Fechas agotadas' : 'Fecha agotada');
    set('camp-waitlist-title', '¿Te has quedado sin plaza?');
    set('camp-waitlist-lead', 'Esta fecha está completa. Déjanos tu aviso y te escribimos por WhatsApp en cuanto abramos la siguiente, antes de publicarla.');
    const btn = document.getElementById('camp-waitlist-btn');
    if (btn) btn.href = waHref('¡Hola! El Surf Camp está agotado. Quiero que me aviséis de la siguiente fecha.');
  }
}
