...
export async function openClassPicker(opts) {
  ...
  // Estado
  let view = 'select';        // 'select' | 'auth'
  let dateOffset = 0;
  let selectedDate = getDateRange(0)[0];
  ...
  function openAttendeePicker(cls) {
    ...
    const familyOpts = family.map(m => `
      <label class="cp-att-opt">
        <input type="radio" name="cp-att" value="fam:${m.id}">
    ...
    panel.innerHTML = `
      <div class="cp-attendee-box">
        ...
        <p class="cp-att-sub">${esc(cls.title || TYPE_LABELS[classType])} · ${fmtTime(cls.time_start)}</p>
        <div class="cp-att-opts">
          <label class="cp-att-opt"><input type="radio" name="cp-att" value="self" checked><span>Yo mismo</span></label>
          ${familyOpts}
          <label class="cp-att-opt"><input type="radio" name="cp-att" value="new"><span>${family.length || user ? 'Otra persona (nueva)' : 'Añadir acompañante'}</span></label>
        </div>
        ...
        <div class="cp-att-actions">
          <button class="btn red" id="cp-att-confirm">Añadir clase</button>
          <button class="btn line" id="cp-att-cancel">Cancelar</button>
        </div>
      </div>`;
    overlay.appendChild(panel);
    const guestForm = panel.querySelector('.cp-guest-form');
    panel.querySelectorAll('input[name="cp-att"]').forEach(r => r.addEventListener('change', () => {
      guestForm.style.display = panel.querySelector('input[name="cp-att"]:checked').value === 'new' ? '' : 'none';
    }));
    panel.querySelector('#cp-att-cancel').onclick = () => panel.remove();
    panel.querySelector('#cp-att-confirm').onclick = async () => {
      const val = panel.querySelector('input[name="cp-att"]:checked').value;
      let attendee;
      if (val === 'self') {
        attendee = { kind: 'self', label: 'Yo' };
      } else if (val.startsWith('fam:')) {
        const id = val.slice(4);
        const m = family.find(x => x.id === id);
        attendee = { kind: 'family', family_member_id: id, label: m ? m.full_name : 'Familiar' };
      } else {
  ...
  async function render() {
    ...
    html += `<div class="date-strip cp-strip">
        <button class="date-strip-arrow" id="cp-prev">&lsaquo;</button>
        ${dates.map(d => `<button class="date-strip-day ${d === selectedDate ? 'active' : ''}" data-date="${d}">${dayLabel(d)}</button>`).join('')}
        <button class="date-strip-arrow" id="cp-next">&rsaquo;</button>
      </div>`;
    ...
    html += `<div class="cp-footer">
        <span class="cp-footer-note">${left > 0
          ...
          : 'Has asignado todas tus clases.'}${rest > 0 ? ` Pagas ${deposit}€ ahora y ${rest}€ en tu primera clase.` : ''}</span>
        <div class="cp-footer-btns">
          <button class="btn line" id="cp-cancel">Cancelar</button>
          <button class="btn red" id="cp-confirm">Reservar y pagar · ${deposit}€</button>
        </div>
      </div>`;
    ...
    modal.querySelector('#cp-confirm').onclick = async (e) => {
    ...
    modal.querySelector('#cp-prev').onclick = () => { dateOffset = Math.max(dateOffset - 10, 0); render(); };
    modal.querySelector('#cp-next').onclick = () => { dateOffset += 10; render(); };
    modal.querySelectorAll('.date-strip-day').forEach(b => b.onclick = () => { selectedDate = b.dataset.date; render(); });
