  ...
  function openAttendeePicker(cls) {
    ...
    const familyOpts = family.map(m => {
      ...
      <label class="cp-att-opt${dis ? ' cp-att-disabled' : ''}">
        <input type="checkbox" class="cp-att-cb" value="fam:${m.id}" ${dis ? 'disabled' : ''}>
    ...
    panel.innerHTML = `
      <div class="cp-attendee-box">
        ...
        <div class="cp-att-opts">
          <label class="cp-att-opt${selfBooked ? ' cp-att-disabled' : ''}"><input type="checkbox" class="cp-att-cb" value="self" ${selfBooked ? 'disabled' : ''}><span>Yo mismo${selfBooked ? ' · ya en esta clase' : ''}</span></label>
          ${familyOpts}
        </div>
    ...
    panel.querySelector('#cp-add-guest').onclick = () => {
      ...
      block.querySelector('.cp-guest-remove').onclick = () => block.remove();
